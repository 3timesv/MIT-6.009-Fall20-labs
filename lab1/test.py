...
def test_load():
    result = lab.load_image(os.path.join(
...
def test_inverted_1():
    im = lab.load_image(os.path.join(
        TEST_DIRECTORY, 'test_images', 'centered_pixel.png'))
...
def test_inverted_2():
    ...
    expected = {
        "height": 1,
        "width": 4,
        "pixels": [255 - pixel for pixel in im["pixels"]]
    }
    compare_images(result, expected)
...
def test_correlated():
    ...
    print("result_len", len(result["pixels"]))
    print("exp_len", len(expected["pixels"]))
    print("RESULT", result)
    compare_images(result, expected)
...
def test_blurred_centered_pixel():
    ...
    res1 = lab.blurred(im, 3)
    res2 = lab.blurred(im, 5)

    compare_images(res1, exp1)
    compare_images(res2, exp2)
