...
def make_dataset_loader():
    ...
    def load_dataset(name):
        if name not in cache:
            nodes_name = os.path.join(
                TEST_DIRECTORY, 'resources', f'{name}.nodes')
            ways_name = os.path.join(
                TEST_DIRECTORY, 'resources', f'{name}.ways')
...
def test_mit_short_nodes_00():
    ...
    compare_result_expected(load_dataset(
        'mit'), (loc1, loc2), expected_path, 'short', True)
...
def test_mit_short_00():
    ...
    compare_result_expected(load_dataset(
        'mit'), (loc1, loc2), expected_path, 'short')
...
def test_mit_short_04():
    ...
    # Parking Lot - end of a oneway and not on any other way
    loc1 = (42.3575, -71.0956)
...
def test_mit_fast_00():
    ...
    compare_result_expected(load_dataset(
        'mit'), (loc1, loc2), expected_path, 'fast')
